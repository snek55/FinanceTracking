namespace FinanceTracking.BlazorCustomComponents
{
	using Humanizer;
	...
	public sealed class InputSelectEnum<TEnum> : InputBase<TEnum>
	{
		...
		protected override void BuildRenderTree(RenderTreeBuilder builder)
		{
			...
			builder.AddAttribute(4, "onchange", EventCallback.Factory.CreateBinder<string>(this, value => this.CurrentValueAsString = value, 
				this.CurrentValueAsString, null));

			// Add an option element per enum value
			var enumType = this.GetEnumType();
			foreach (TEnum value in Enum.GetValues(enumType))
			{
				builder.OpenElement(5, "option");
				builder.AddAttribute(6, "value", value.ToString());
				builder.AddContent(7, this.GetDisplayName(value));
